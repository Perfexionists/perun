...
def assert_before_add(helpers, path, commit, valid_profile):
    ...
    with helpers.open_index(path, commit) as index_handle:
        ...
        assert not helpers.exists_profile_in_index_such_that(index_handle,
...
def test_add(helpers, pcs_full, valid_profile_pool):
    ...
    for valid_profile in valid_profile_pool:
        ...
        before_number_of_entries = assert_before_add(helpers, obj_path, working_commit, valid_profile)
        ...
        after_number_of_entries = assert_after_valid_add(helpers, obj_path, working_commit, valid_profile)
...
def test_add_no_minor(helpers, pcs_full, valid_profile_pool):
    ...
    for valid_profile in valid_profile_pool:
        # Check that the profile was NOT in the index before
        before_number_of_entries = assert_before_add(helpers, obj_path, head, valid_profile)

        commands.add(valid_profile, None)

        # Now check, that the profile was successfully added to index, and its entry is valid
        after_number_of_entries = assert_after_valid_add(helpers, obj_path, head, valid_profile)
        assert before_number_of_entries == (after_number_of_entries - 1)
...
def test_add_wrong_profile(helpers, pcs_full, error_profile_pool):
    ...
    for error_profile in error_profile_pool:
        before_number_of_entries = assert_before_add(helpers, pcs_full.path, head, error_profile)
        ...
        after_number_of_entries = assert_after_invalid_add(helpers, pcs_full.path, head, error_profile)
...
def test_add_existing(helpers, pcs_full, valid_profile_pool, capsys):
    ...
    for valid_profile in valid_profile_pool:
        ...
        middle_number_of_entries = assert_after_valid_add(helpers, obj_path, head, valid_profile)
