...
def test_scatter_plot_no_models():
    ...
    complexity_prof = test_utils.load_profile('full_profiles', 'prof-2-complexity-2017-03-20-21-40-42.perf')

    # Create graphs from one profile without models
    graphs = scatter.create_from_params(complexity_prof, 'amount', 'structure-unit-size',
                                        'structure-unit-size', 'amount [us]',
                                        "Plot of 'amount' per 'structure-unit-size'")
...
def test_scatter_plot_cli(pcs_full_no_prof):
    ...
    runner = CliRunner()
    result = runner.invoke(cli.show, [profile, 'scatter', '--of=amount',
...
def test_scatter_plot_cli_errors(pcs_full_no_prof):
    ...
    result = runner.invoke(cli.show, [profile, 'scatterr', '--of=amount',
    ...
    result = runner.invoke(cli.show, [profile, 'scatter', '--of=amou',
                                      '--per=structure-unit-size'])
