...
def flattened_values(
    ...
    if isinstance(root_value, dict):
        ...
        if isinstance(root_key, str):
            ...
            yield root_key, ":".join(
                map(str, map(operator.itemgetter(1), nested_values))
            )
        ...
        # Lists are merged as comma separated keys
            ...
            yield root_key, ",".join(
                ":".join(
                    str(nested_value[1]) for nested_value in flattened_values(i, lv)
                )
...
def _all_fields_of(
    item_generator: Iterable[tuple[int, dict[str, Any]]]
) -> Iterable[str]:
    ...
    for level_idx, key_level in enumerate(key_hierarchy):
        if key_level == "" and isinstance(resource, (list, set)):
            ...
            for item in resource:
                for result in all_key_values_of(
                    item, ":".join(key_hierarchy[level_idx + 1 :])
                ):
                    yield result
            return
...
def _unique_values_generator(
    key: str, blocks_gen: Iterable[tuple[Any, dict[str, Any]]]
) -> Any:
