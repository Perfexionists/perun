...
def valid_step_size(step: float, range: tuple(float, float)) -> bool:
    """
    This method represents click callback option method.
    ...
    :param range: tuple of length of the entered gamma range
    ...
    if step < range_length:
        ...
        raise click_exp.BadOptionUsage(
            ...
            f'({step:.5f} >= {range_length:.5f}) for range {range[0]}:{range[1]}'
        )
