...
def estimator_settings(ctx, **kwargs):
    ...
    between explanatory variable **y** and the response variable **X**. This mode of kernel regression
    postprocessor calculates the conditional mean **E[y|X] = m(X)**, where **y = m(X) +** :math:`\epsilon`.
    Variable **X** is represented in the postprocessor by <per-key> option and the variable **y** is
    represented by <of-key> option.

    **Regression Estimator <reg-type>**:

         This mode offer two types of *regression estimator* <`reg-type`>. *Local Constant (`ll`)* type of
         regression provided by this mode is also known as *Nadaraya-Watson* kernel regression:

            **Nadaraya-Watson**: expects the following conditional expectation: **E[y|X] = m(X)**, where
            function **m(*)** represents the regression function to estimate. Then we can alternatively write
            the following formula: **y = m(X) +** :math:`\epsilon`, **E** (:math:`\epsilon`) **= 0**. Then we can
            suppose, that we have the set of independent observations {(:math:`{x_1}`, :math:`{y_1}`), ...,
            (:math:`{x_n}`, :math:`{y_n}`)} and the **Nadaraya-Watson** estimator is defined as:

                .. math::

                       m_{h}(x) = \sum_{i=1}^{n}K_h(x - x_i)y_i / \sum_{j=1}^{n}K_h(x - x_j)

            where :math:`{K_h}` is a kernel with bandwidth *h*. The denominator is a weighting term with sum 1.
            It easy to see that this kernel regression estimator is just a weighted sum of the observed responses
            :math:`{y_i}`. There are many other kernel estimators that are various in compare to this presented
            estimator. However, since all are asymptotic equivalently, we will not deal with them closer. **Kernel
            Regression** postprocessor works in all modes only with **Nadaraya-Watson** estimator.

        The second supported *regression estimator* in this mode of postprocessor is *Local Linear (`lc`)*. This
        type is an extension of that which suffers less from bias issues at the edge of the support.

            **Local Linear**: estimator, that offers various advantages compared with other kernel-type estimators,
            such as the *Nadaraya-Watson* estimator. More precisely, it adapts to both random and fixed designs, and to
            various design densities such as highly clustered designs and nearly uniform designs. It turns out that
            the *local linear* smoother repairs the drawbacks of other kernel regression estimators. An regression
            estimator *m* of *m* is a linear smoother if, for each *x*, there is a vector :math:`l(x) = (l_1(x), ...,
            l_n(x))^T` such that:
            ...
            For a better imagination, there is an interesting fact, that the following estimators are linear
            smoothers too: *Gaussian process regression*, *splines*.

    **Bandwidth Method <bandwidth-method>**:

        As has been said in the general description of the *kernel regression*, once of the most important
        factors of the resulting estimate is the kernel **bandwidth**. When the inappropriate value is selected
        may occur to *under-laying* or *over-laying* fo the resulting kernel estimate. Since the bandwidth of
        the kernel is a free parameter which exhibits a strong influence on the resulting estimate postprocessor
        offers the method for its selection. Two most popular data-driven methods of bandwidth selection that
        have desirable properties are *least-squares cross-validation* (`cv_ls`) and the *AIC-based* method of
        *Hurvich et al. (1998)*, which is based on minimizing a modified *Akaike Information Criterion* (`aic`):

            **Cross-Validation Least-Squares**: determination of the optimal kernel bandwidth for kernel
            regression is based on minimizing

            .. math::

                CV(h) = n^{-1} \sum_{i=1}^{n}(Y_i - g_{-i}(X_i))^2,

            where :math:`g_{-i}(X_i)` is the estimator of :math:`g(X_i)` formed by leaving out the *i-th*
            ...
            **Hurvich et al.'s** (1998) approach is based on the minimization of
            ...
            with :math:`g(X_i)` being a non-parametric regression estimator and *H* being an *n x n* matrix of
            kernel weights with its *(i, j)-th* element given by :math:`H_{ij} = K_h(X_i, X_j) / \sum_{l=1}^{n}
            K_h(X_i, X_l)`, where :math:`K_h(*)` is a generalized product kernel.

        Both methods for kernel bandwidth selection the *least-squared cross-validation* and the *AIC* have been
        shown to be asymptotically equivalent.

    .. _EstimatorSettings: https://www.statsmodels.org/dev/generated/statsmodels.nonparametric.kernel_density.EstimatorSettings.html
    .. _StatsModels: https://www.statsmodels.org/dev/generated/statsmodels.nonparametric.kernel_regression.KernelReg.html#statsmodels.nonparametric.kernel_regression.KernelReg

    The remaining options at this mode of kernel regression postprocessor are described within usage to it and you
    can see this in the list below. All these options are parameters to *EstimatorSettings* (see EstimatorSettings_),
    that optimizing the kernel bandwidth based on the these specified settings.
...
def user_selection(ctx, **kwargs):
    ...
    This mode of kernel regression postprocessor is very similar to *estimator-settings* mode. Also offers two
    types of *regression estimator* <reg-type> and that the *Nadaraya-Watson* estimator, so known as *local-
    constant* (`lc`) and the *local-linear* estimator (`ll`). Details about these estimators are available in
    :ref:`postprocessors-kernel-regression-estimator_settings`. In contrary to this mode, which selected a kernel
    bandwidth using the *EstimatorSettings* and chosen parameters, in this mode the user itself selects a kernel
    bandwidth <bandwidth-value>. This value will be used to execute the kernel regression. The value of kernel
    bandwidth in the resulting estimate may change occasionally, specifically in the case, when the bandwidth
    value is too low to execute the kernel regression. Then will be a bandwidth value approximated to the
...
def method_selection(ctx, **kwargs):
    ...
    The last method from a group of three methods based on a similar principle. *Method-selection* mode
    offers the same type of *regression estimators* <reg-type> as the first two described methods. The first
    supported option is `ll`, which represents the *local-linear* estimator. *Nadaraya-Watson* or *local constant*
    estimator represents the second option for <reg-type> parameter. The more detailed description of these
    estimators is located in :ref:`postprocessors-kernel-regression-estimator_settings`. The difference between
    this mode and the two first modes is in the way of determination of a kernel bandwidth. In this mode are offered
    two methods to determine bandwidth. These methods try calculated an optimal bandwidth from predefined formulas:
        ...
        **Scotts's Rule** of thumb to determine the smoothing bandwidth for a kernel estimation. It
        ...
        regression too. Typically produces a larger bandwidth and therefore it is useful for estimating
        a gradual trend:
        ...
        **Silverman's Rule** of thumb to determine the smoothing bandwidth for a kernel estimation. Belongs to
        most popular method which uses the *rule-of-thumb*. Rule is originally designs for *density estimation*
        and therefore uses the normal density as a prior for approximating. For the necessary estimation of
        the :math:`\sigma` of X <per-key> he proposes a robust version making use of the InterquartileRange_.
        If the true density is uni-modal, fairly symmetric and does not have fat tails, it works fine:
...
@click.option('--smoothing-method', '-sm', type=click.Choice(_SMOOTHING_METHODS), default=_SMOOTHING_METHODS[0],
              help=('Provides kernel smoothing methods to executing non-parametric regressions: `local-polynomial` '
                    ...
                    'perform a local-linear regression using a gaussian (normal) kernel; and `spatial-average` perform'
                    'a Nadaraya-Watson regression on the data (so called local-constant regression) using a user-'
                    'provided kernel.'))
@click.option('--bandwidth-method', '-bm', type=click.Choice(methods.BW_SELECTION_METHODS),
              default=methods.BW_SELECTION_METHODS[0],
...
def kernel_smoothing(ctx, **kwargs):
    ...
    This mode of kernel regression postprocessor implements non-parametric regression using different kernel
    methods and different kernel types. The calculation in this mode can be split into three parts. The first
    part is represented by the *kernel type*, the second part by *bandwidth computation* and the last part is
    represented by *regression method*, which will be used to interleave the given resources. We will look
    gradually at individual supported options in the each part of computation.

        **Kernel Type <kernel-type>**:

        In non-parametric statistics a *kernel* is a weighting function used in estimation techniques. In *kernel
        regression* is used to estimate the conditional expectation of a random variable. As has been said, *kernel
        width* must be specified when running a non-parametric estimation. The *kernel* in view of mathematical
        definition is a non-negative real-valued integrable function *K*. For most applications, it is desirable
        to define the function to satisfy two additional requirements:
            ...
            **Symmetry**
            ...
            for all values of u. The second requirement ensures that the average of the corresponding distribution
            is equal to that of the sample used. If *K* is a kernel, then so is the function :math:`K^*` defined
            by :math:`K^*(u) = \lambda K (\lambda u)`, where :math:`\lambda > 0`. This can be used to select a scale
            that is appropriate for the data. This mode offers several types of kernel functions:
            ...
            Efficiency is defined as :math:`\sqrt{\int{}{}u^2K(u)du}\int{}{}K(u)^2du`, and its measured to the
            *Epanechnikov* kernel.

        **Smoothing Method <smoothing-method>**:

        *Kernel-Smoothing* mode of this postprocessor offers three different non-parametric regression methods to
        execute *kernel regression*. The first of them is called *spatial-average* and perform a *Nadaraya-Watson*
        regression (i.e. also called local-constant regression) on the data using a given kernel:
        ...
        *Local-Constant* regression was also described in :ref:`postprocessors-kernel-regression-estimator_settings`.
        The second supported regression method by this mode is called *local-linear*. Compared with previous method,
        which offers computational with different types of kernel, this method has restrictions and perform *local-
        linear* regression using only *Gaussian (Normal)* kernel. The *local-constant* regression was described in
        :ref:`postprocessors-kernel-regression-estimator_settings` and therefore will not be given no further
        attention to it. *Local Polynomial* regression is the last method in this mode and perform regression in *N-D*
        using a user-provided  kernel. The *local-polynomial* regression is the function that minimizes, for each
        position:
        ...
        where *K(x)* is the kernel such that *E(K(x)) = 0*, *q* is the order of the fitted polynomial
        <polynomial-order>, :math:`P_q(x)` is a polynomial or order *q* in *x*, and *h* is the bandwidth
        of the method. The polynomial :math:`P_q(x)` is of the form:
    ...
    The last part of the calculation is the *bandwidth* computation. This mode offers to user enter the value
    directly with use of parameter <bandwidth-value>. The parameter <bandwidth-method> offers to user the selection
    from the two methods to determine the optimal bandwidth value. The supported methods are *Scotts's Rule* and
    *Silverman's Rule*, which are described in :ref:`postprocessors-kernel-regression-method_selection`. This
    parameter cannot be entered in combination with <bandwidth-value>, then will be ignored and will be accepted value
    from <bandwidth-value>.
    """
    # update the current set of params with the selected mode of kernel regression
...
def kernel_ridge(ctx, **kwargs):
    ...
    :ref:`postprocessors-kernel-regression-estimator_settings`. While the previous modes provided the methods to
    determine the optimal bandwidth with different ways, this method provides a little bit different way. From a given
    range of potential bandwidths <gamma-range> try to select the optimal kernel bandwidth with use of *leave-one-out
    cross-validation*. This approach was described in :ref:`postprocessors-kernel-regression-estimator_settings`, where
    was introduced the *least-squares cross- validation* and it is a modification of this approach. *Leave-one-out
    cross validation* is K-fold_ cross validation taken to its logical extreme, with *K* equal to *N*, the number of
    data points in the set. The original *gamma-range* will be divided on the base of size the given step <gamma-step>.
    The selection of specific value from this range will be executing by minimizing mean-squared-error_ in
    *leave-one-out cross-validation*. The selected *bandwidth-value* will serves for *gaussian* kernel in resulting
    estimate: :math:`K(x, y) = exp(-gamma * ||x-y||^2)`.
    """
    ...
    **kernel**, that have the role of weighted function, to each observation point in the dataset.
    ...
    ('local') data point X to a set of neighboring data points X.
        ...
        **Kernel Selection** does not important from an asymptotic point of view. It is appropriate
        to choose the **optimal** kernel since this group of the kernels are continuously on the whole
        definition field and then the estimated regression function inherit smoothness of the kernel.
        For example, a suitable kernels can be the **epanechnikov** or **normal** kernel. This
        postprocessor offers the **kernel selection** in the **kernel-smoothing** mode, where are available
        five different types of kernels. For more information about these kernels or this kernel regression
        mode you can see :ref:`postprocessors-kernel-regression-kernel_smoothing`.

        **Bandwidth Selection** is the most important factor at each approach of kernel regression, since
        this value significantly affects the smoothness of the resulting estimate. In case, when is choose the
        inappropriate value, in the most cases can be expected the following two situations. The **small**
        bandwidth value reproduce estimated data and vice versa, the **large** value leads to over-leaving, so
        to average of the estimated data. Therefore are used the methods to determine the bandwidth value.
        One of the most widespread and most commonly used methods is the **cross-validation** method. This
        method is based on the estimate of the regression function in which will be omitted *i-th* observation.
        In this postprocessor is this method available in the **estimator-setting** mode. Another methods to
        determine the bandwidth, which are available in the remaining modes of this postprocessor are **scott**
        and **silverman** method. More information about these methods and its definition you cas see in the
        part :ref:`postprocessors-kernel-regression-method_selection`.

    This postprocessor in summary offers five different modes, which does not differ in the resulting estimate,
    but in the way of computation the resulting estimate. Better said, it means, that the result of each mode
    is the **kernel estimate** with relevant parameters, selected according to the concrete mode. In short we
    will describe the individual methods, for more information about it, you can visit the relevant parts of
    documentation:
    ...
    For more details about this approach of non-parametric analysis refer to :ref:`postprocessors-kernel-regression`.
    """
...
# - kernel-ridge: set bandwidth by minimizing mean-squared error in leave-one-out cross validation from given range
