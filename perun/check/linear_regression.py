...
def exec_linear_regression(
    ...
    if baseline_model.type == "linear" or baseline_model.type == "constant":
        if (
            helpers.abs_in_absolute_range(gradient, threshold)
            and isinstance(diff_b0, float)
            and helpers.abs_in_relative_range(diff_b0, intercept, 0.05)
            and abs(diff_b0 - intercept) < 0.000000000001
        ):
            change_type = "constant"
        elif helpers.abs_in_relative_range(linear_diff_b1, gradient, 0.3) and r_value ** 2 > 0.95:
            change_type = "linear"
    else:
        if (
            helpers.abs_in_absolute_range(gradient, threshold)
