...
from perun.thirdparty.pyqt_fit_port.utils import finite, large_float
...
def variance_bandwidth(factor, xdata):
    ...
    """
    data_covariance = np.atleast_2d(np.cov(xdata, rowvar=True, bias=False))
    sq_bandwidth = data_covariance * factor * factor
    return sq_bandwidth


def silverman_covariance(xdata, _=None):
    r"""
    ...
    d, n = xdata.shape
    return variance_bandwidth(np.power(n * (d + 2.0) / 4.0, -1.0 / (d + 4.0)), xdata)


def scotts_covariance(xdata, _=None):
    r"""
    ...
    xdata = np.atleast_2d(xdata)
    d, n = xdata.shape
    return variance_bandwidth(np.power(n, -1.0 / (d + 4.0)), xdata)
...
class BotevBandwidth:
    ...
    def __init__(self, N=None, **kword):
        if "lower" in kword or "upper" in kword:
            print(
                "Warning, using 'lower' and 'upper' for botev bandwidth is "
